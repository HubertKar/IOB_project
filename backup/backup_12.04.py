...
def crate_U_model(img_height, img_width,img_channels, num_classes = 1, show_summary = False, node_num = 16): # Tworzenie modelu siecu konwolucyjnej (U-net)
    ...
    #Contraction path
    c1 = tf.keras.layers.Conv2D(node_num, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same')(inputs) #Pierwsza warstwa konwolucyjna
...
class Index:
    def next(self, event): # Callback do przycisku
        ...
        prediction = u_net_model.predict(sample_image[tf.newaxis, ...])[0]
...
model_choice = input("[0] - stórz nowy model,  [1] - wczytaj model: ")
if model_choice == "1":
   print("Dostępne modele: ")
   print(os.listdir("saved_model"))
   model_choice = input("Wybierz model (wpisz bez .keras): ")

u_net_model = crate_U_model(IMG_HEIGHT, IMG_WIDTH, IMG_CHANELS, num_classes=1, show_summary=True) # Tworzenie modelu
...
if model_choice == '0':
   ...
   u_net_model.load_weights("saved_model/" + model_choice + ".keras") # Załaduj wagi do warst modelu z pliku
